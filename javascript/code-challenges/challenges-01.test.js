...
const addOne = (arr) => {
  // Create a local array to store the incremented values
  const results = [];

  // Use forEach to iterate over the input array
  arr.forEach((number) => {
    // Increment the current number and push it to the results array
...
const addExclamation = (arr) => {
  // Define a local array to hold the modified strings
  const shoutedStrings = [];

  // Loop through each string in the input array
  arr.forEach((str) => {
    // Concatenate the exclamation point and push to the local array
    shoutedStrings.push(str + '!');
  });

  // Return the new array
  return shoutedStrings;
};
...
const allUpperCase = (arr) => {
  // 1. Create a local array to store the uppercase strings
  const upperCaseArray = [];

  // 2. Loop over the input array
  arr.forEach((str) => {
    // 3. Convert each string and push it into our local array
...
const greeting = (word) => {
  // Convert to uppercase and add an exclamation point
  return word.toUpperCase() + '!';
};

const speaker = (words, callback) => {
  // 1. Create the local array
  const results = [];

  // 2. Loop through the words
  words.forEach((item) => {
    // 3. Apply the callback function to each item
    // and push the result into our array
...
const addValues = (arr, value) => {
  // Directly push the value into the array provided
  arr.push(value);
};

const addNumbers = (num, arr, times, callback) => {
  // We need to run the callback a specific number of times
  // A standard for loop is perfect for this "count-based" logic
  for (let i = 0; i < times; i++) {
    callback(arr, num);
  }

  // Return the array that has now been modified by the callback
  return arr;
};
...
const createList = (availableItems) => {
  // 1. Initialize the local array for your grocery list
  const groceryList = [];

  // 2. Use forEach to iterate over the inventory objects
  availableItems.forEach((item) => {
    // 3. Use an 'if' statement to check the 'available' property
    if (item.available === true) {
      // 4. If true, push the name of the item into the list
